...
def run_worker():
    ...
    while True:
        try:
            ...
            logger.info(f"Acquired job {job_id} for session {session_id}. Starting execution...")
            job_manager.update_job_status(job_id, "running")

            # Update session status to IN_PROGRESS in postgres database
            brain_service.update_session(session_id, {"status": "IN_PROGRESS"})
            ...
            try:
                ...
                final_session = brain_service.get_session(session_id)
                ...
                if session_status == "COMPLETED":
                    ...
                    brain_service.update_session(session_id, {"status": "FAILED"})
